...
def measurements(image_path):
    ...
    # Calibration
    print("Attempting automatic calibration (A4 detection).")
    scale_cm_per_px = get_calibration_scale(image, allow_manual=False)
    if scale_cm_per_px is None:
        raise RuntimeError("Calibration failed. Please ensure an A4 paper is clearly visible in the image.")
